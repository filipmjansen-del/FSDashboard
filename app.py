...
def show_kpi_workspace(industry: str, kpi_name: str):
    ...
    if kpi.empty:
        st.warning(
            "This KPI currently contains no observations."
        )
        return

    valid = (
        kpi
        .dropna(subset=["KPI_Value"])
        .copy()
    )
    ...
    if not all_years:
        st.warning(
            "No years are available for this KPI."
        )
        return
    ...
    with st.expander(
        ...
        st.code(
            KPI_REGISTRY[kpi_name].get(
                "formula_label",
                "Formula definition not provided.",
            ),
            language=None,
        )
    ...
    with overview_tab:
        ...
        yr = valid[
            valid["ÅR"] == selected_year
        ].copy()

        represented = (
            kpi[
                kpi["ÅR"] == selected_year
            ]["regnr"]
            .nunique()
        )
        ...
        c1.metric(
            f"{entity_plural} represented",
            represented,
        )

        c2.metric(
            f"{entity_plural} with complete KPI",
            complete,
        )
        ...
        st.subheader(
            f"Distribution across {entity_plural.lower()}"
        )
